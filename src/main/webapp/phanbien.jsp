			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<div class="header">
								<h4 class="title">Danh sách đề tài</h4>
							</div>
							<div class="content table-responsive table-full-width">
								<table class="table table-striped">
									<thead>
										...
										<th>Giảng viên phản biện hiện tại</th>
										<th>Chọn giảng viên mới</th>
									</thead>
									<tbody>

										<c:forEach items="${listphanbien}" var="dt">
											<tr>

												<td>${dt.getDeTai().getMaDeTai()}</td>
												<td>${dt.getDeTai().getTenDeTai()}</td>
												<td>${dt.getGiangVien().getHoten()}</td>
												<td><div class="col-md-12">
														<form action="xac-nhan-giang-vien-phan-bien" method="get">
															<select name="giangvien"
																class="form-select col-md-12 form-control border-input"
																aria-label="Default select example">
																<c:forEach items="${listgv}" var="gv">
																	<option value="${gv.getId()}">${gv.getHoten()}</option>
																</c:forEach>
															</select>
															<input name="madt" type="hidden" value="${dt.getDeTai().getMaDeTai()}"/>
															<button type="submit"
																class="btn btn-info btn-fill btn-wd">Xác nhận
																thay đổi</button>
														</form>
													</div></td>
												<td></td>
								...
								<c:if test="${mess != null}">
									<h3 class="" style="text-align: center; color: #f44336;">${mess}</h3>
