			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<div class="header">
								<h4 class="title">Chi tiết đề tài ${madt}</h4>
							</div>
									...
									<tbody>
										...
										<c:forEach items="${listnhom}" var="nh">
											<tr>
												...
												<td>${nh.getLienHe()}</td>
												<td><a
													href="xem-chi-tiet-nhom?manhom=${nh.getMaNhom()}">Chi
														tiết</a></td>
												<td>
												<c:if test="${trangthai ==0}">
												<div class="text-center">
														<a href="duyet-nhom?manhom=${nh.getMaNhom()}&madt=${madt}">
															<button type="submit"
																class="btn btn-info btn-fill btn-wd">Duyệt</button>
														</a>
													</div>
													</c:if>
													<c:if test="${trangthai ==1}">
													<p class="category" style="color: #3aa64c">Đã duyệt</p>
													</c:if>
													</td>
											</tr>
										</c:forEach>
